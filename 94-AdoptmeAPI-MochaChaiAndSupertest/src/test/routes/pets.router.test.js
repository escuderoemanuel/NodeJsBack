...
describe('Testing AdoptmeAPI', function () {
  ...
  describe('Pet Tests', function () {
    ...
    it('2. Al crear una mascota sólo con los datos elementales. Se debe corroborar que la mascota creada cuente con una propiedad adopted : false', async () => {
      ...
      // expect(responsePet).to.have.property('adopted', false); // Esto resume los dos anteriores
    });
    ...
    it('4. Al obtener a las mascotas con el método GET, la respuesta debe tener los campos status y payload. Además, payload debe ser de tipo arreglo', async () => {
      ...
      // expect(response._body.payload).to.be.an('array'); // Lo mismo que el anterior
      // expect(Array.isArray(response._body.payload)).to.be.equal(true); // Lo mismo que el anterior
      // expect(response._body).to.have.property('payload').that.is.an('array'); // Puede ser una alternativa para resumir los dos pasos
    });
    ...
    it('El método DELETE debe poder borrar la última mascota agregada, ésto se puede alcanzar agregando a la mascota con un POST, tomando el id, borrando la mascota  con el DELETE, y luego corroborar si la mascota existe con un GET', async () => {
      ...
      expect(petDeleted._body).not.to.have.property('payload') // Opcional para verificar que no exista un payload
    })
